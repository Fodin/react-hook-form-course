...
Реальные формы часто требуют работы с сервером: проверка доступности username, загрузка данных для
редактирования, отправка формы с обработкой ошибок. В этом уровне вы изучите все аспекты асинхронной
работы с формами.
...
> **`isLoading`** — свойство `formState`, которое равно `true` только когда `defaultValues` является
...
Если данные формы приходят из внешнего источника (например, из SWR, React Query или Redux),
используйте опцию `values`. Форма будет автоматически обновляться при изменении `values`:
...
**Почему это ошибка:** Сеть может отказать, и пользователь должен увидеть понятное сообщение об
ошибке.
