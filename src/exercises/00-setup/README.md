...
1. **🚀 Производительность** — минимальное количество ререндеров благодаря неконтролируемым компонентам
...
В контролируемом подходе каждое изменение вызывает ререндер. В неконтролируемом — React Hook Form напрямую работает с DOM.
...
- Нарушается семантика HTML — кнопка без `type` по умолчанию имеет `type="submit"`, но лучше явно указывать
